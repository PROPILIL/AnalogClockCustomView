package com.propil.clockview.views
...
class AnalogClock @JvmOverloads constructor(
    ...
    // vals for elements paint (use it fo init)
    ...
    //sizes for elements (ust it for drawLines)
    ...
    private var circleColor = ContextCompat.getColor(getContext(), R.color.clock_ring_color)
    private var scaleColor = ContextCompat.getColor(getContext(), R.color.clock_big_scale_color)
    private var minuteHandColor =
        ContextCompat.getColor(getContext(), R.color.clock_minute_hand_color)
    private var secondHandColor =
        ContextCompat.getColor(getContext(), R.color.clock_second_hand_color)
    private var hourHandColor = ContextCompat.getColor(getContext(), R.color.clock_hour_hand_color)
    private var numberColor = ContextCompat.getColor(getContext(), R.color.clock_numbers_color)


    // use it to position view on a screen
    ...
    init {
        val attrsArray = context.obtainStyledAttributes(attributeSet, R.styleable.AnalogClock)
        //init colors for elements in customView
        circlePaint.color =
            attrsArray.getColor(R.styleable.AnalogClock_clock_ring_color, circleColor)
        bigScalePaint.color =
            attrsArray.getColor(R.styleable.AnalogClock_clock_big_scale_color, scaleColor)
        smallScalePaint.color =
            attrsArray.getColor(R.styleable.AnalogClock_clock_small_scale_color, scaleColor)
        hourPaint.color =
            attrsArray.getColor(R.styleable.AnalogClock_clock_hour_color, hourHandColor)
        minutePaint.color =
            attrsArray.getColor(R.styleable.AnalogClock_clock_minute_color, minuteHandColor)
        secondPaint.color =
            attrsArray.getColor(R.styleable.AnalogClock_clock_second_color, secondHandColor)
        textPaint.color = attrsArray.getColor(R.styleable.AnalogClock_clock_text_color, numberColor)

        //to drop unused data in view
        attrsArray.recycle()

        // init paint styles for elements
        initPaints()

        //to get actual phoneWidth form displayMetrics
    ...
    private fun drawText(canvas: Canvas?) {
        //TODO: Change text rotation
        canvas?.withSave {
            //rotate text for 30 degrees
    ...
    companion object {
        private const val HAND_MOVE_DEGREE = 6f // 360 / 60
        private const val HOUR_MOVE_DEGREE = 30f // 360 / 60


        //use it to paint the hands
        var hourDegrees = 0f
